...
if __name__ == "__main__":
    lista = []
    print(f"Vamos a crear un objeto con tu informacion personal")
    persona1 = None
    while True:
        error = False
        try:
            nombre = input("Introduce un nombre (Solo caracteres alfabeticos introduce 0 para salir)\n")
            if nombre == "0":
                break
        ...
        if not error:
            ...
            while True:
                crear = False
                nuevo_a = input("Quieres introducir otra persona? S/N\n").upper()
                if nuevo_a == "N":
                    break
                elif nuevo_a:
                    crear = True
    ...
    for i in lista:
        i.Mostrar_Dades()
        print(f"Es major de edad? ----->  {i.EsMajorEdat()}")
        print(i)
